...
def predict_label_demo(text):
    """
    :param text:输入的文本
    :return: 返回标签
    """
    label = "label"
    model = BertForSequenceClassification.from_pretrained(label)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model.to(device)
    tokenizer = BertTokenizer.from_pretrained(label)
    ...
    prediction = Softmax(prediction.logits)
    _, pred = torch.max(prediction, dim=1)
    if _ < 0.95:
        pred = -1
        prediction = MAPPING_LABEL2CATEGORY[pred]
    else:
        prediction = MAPPING_LABEL2CATEGORY[pred.cpu().numpy()[0]]
    return prediction
...
if __name__ == '__main__':
    ...
    output = predict_label_demo(text)
    print(output)
